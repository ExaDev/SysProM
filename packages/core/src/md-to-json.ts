...
/**
 * Parse a multi-document Markdown file map (filename → content) into a SysProM
 * document. This is the pure counterpart to the root-level
 * `markdownMultiDocToJson(dir)` fs wrapper: instead of reading the directory,
 * it accepts the file map directly. Keys use `/`-separated paths for nested
 * subsystem folders and grouping directories, matching `renderMultiDoc`.
 * @param files - Map of filename to Markdown content.
 * @returns The parsed SysProM document.
 */
export function parseMultiDoc(files: Record<string, string>): SysProMDocument {
	...
	// Subsystem entries: single-file `.spm.md` and folder-style (README.md +
	// nested files) including those inside grouping directories. Walk the
	// top-level keys, then recurse into grouping folders (keys with `/`).
	function collectSubsystems(prefix: string): void {
		// Group keys by their immediate child under `prefix`.
		const seen = new Set<string>();
		for (const key of Object.keys(files)) {
			if (!key.startsWith(prefix)) continue;
			const rest = key.slice(prefix.length);
			if (rest.includes("/")) {
				// Nested path; the first segment is a child entry.
				const child = rest.slice(0, rest.indexOf("/"));
				if (seen.has(child)) continue;
				seen.add(child);
				const childPrefix = `${prefix}${child}/`;
				// Folder-style subsystem if it contains README.md; otherwise it's
				// a grouping directory — recurse into it.
				if (`${childPrefix}README.md` in files) {
					attachFolderSubsystem(prefix, child, childPrefix);
				} else {
					collectSubsystems(childPrefix);
				}
			} else if (rest.endsWith(".spm.md")) {
				if (seen.has(rest)) continue;
				seen.add(rest);
				attachFileSubsystem(prefix, rest);
			}
		}
	}

	function attachFolderSubsystem(
		prefix: string,
		child: string,
		childPrefix: string,
	): void {
		const idPrefix = child.split("-")[0];
		const parentNode = nodes.find((n) => n.id === idPrefix);
		if (!parentNode) return;
		// Gather all files under childPrefix, stripping that prefix so they
		// become top-level keys for the recursive parse.
		const subFiles: Record<string, string> = {};
		for (const [key, content] of Object.entries(files)) {
			if (key.startsWith(childPrefix)) {
				subFiles[key.slice(childPrefix.length)] = content;
			}
		}
		parentNode.subsystem = parseMultiDoc(subFiles);
	}

	function attachFileSubsystem(prefix: string, child: string): void {
		const idPrefix = child.replace(/\.spm\.md$/, "").split("-")[0];
		const parentNode = nodes.find((n) => n.id === idPrefix);
		if (!parentNode) return;
		const content = files[`${prefix}${child}`];
		parentNode.subsystem = markdownSingleToJson(content);
	}
