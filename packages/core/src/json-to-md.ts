...
/**
 * Render a SysProM document as a multi-document Markdown file map, without
 * touching the filesystem. Returns a map of filename (e.g. "README.md",
 * "DECISIONS.md") to file content. Subsystem nodes are rendered recursively
 * using keys like `${id}-${slug}.spm.md` or nested folder-style maps flattened
 * with `/` separators. Pure: no node:* imports, no disk I/O.
 * @param doc - The SysProM document to render.
 * @param options - Rendering options (diagrams, label mode, layouts).
 * @returns Map of filename to Markdown content.
 */
export function renderMultiDoc(
	doc: SysProMDocument,
	options?: MarkdownRenderOptions,
): Record<string, string> {
	...
	files["README.md"] = generateReadme(doc, fromIdx, nodeMap);

	for (const [fileName, types] of Object.entries(NODE_FILE_MAP)) {
		const hasNodes = doc.nodes.some((n) => types.includes(n.type));
		if (!hasNodes) continue;
		files[`${fileName}.md`] = generateDocFile(
			doc,
			fileName,
			types,
			fromIdx,
	...
	if (
		...
		const clickTargets = options?.diagramLinks
			...
			: undefined;
		files["DIAGRAMS.md"] = generateDiagramsFile(doc, {
			labelMode: options?.labelMode ?? "friendly",
	...
	for (const n of subsystemNodes) {
		...
		// Auto-group when 2+ subsystems share the same type. Uses a folder
		// prefix derived from the type label.
		const shouldGroup =
			(typeCounts.get(n.type) ?? 0) >= 2 && NodeType.is(n.type);
		const folderPrefix = shouldGroup
			? `${NODE_TYPE_LABELS[n.type].toLowerCase().replace(/ /g, "-")}/`
			: "";
		...
		if (fileCounts <= 1) {
			...
			if (lineCount <= 100) {
				files[`${folderPrefix}${slug}.spm.md`] = singleContent;
			} else {
				const subFiles = renderMultiDoc(subDoc);
				for (const [subName, subContent] of Object.entries(subFiles)) {
					files[`${folderPrefix}${slug}/${subName}`] = subContent;
				}
			}
		} else {
			const subFiles = renderMultiDoc(subDoc);
