...
describe("validate", () => {
  ...
  it("decisions affecting domain nodes without must_preserve detected", () => {
    const doc = makeDoc(
      [
        { id: "D1", type: "decision", name: "Dec", options: [{ id: "a", description: "A" }], selected: "a" },
  ...
  it("changes without decision references detected", () => {
    ...
    const result = validate(doc);
    assert.equal(result.valid, false);
    assert.ok(result.issues.some((i) => i.includes("does not reference any decision")));
  });

  it("decisions without options detected", () => {
    const doc = makeDoc([{ id: "D1", type: "decision", name: "Dec", selected: "a" }]);
    const result = validate(doc);
    assert.equal(result.valid, false);
  ...
  it("decisions without selected detected", () => {
    const doc = makeDoc([{ id: "D1", type: "decision", name: "Dec", options: [{ id: "a", description: "A" }] }]);
    const result = validate(doc);
    assert.equal(result.valid, false);
