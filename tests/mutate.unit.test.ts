...
describe("addNode", () => {
  ...
  it("rejects duplicate ID", () => {
    ...
    assert.throws(() => addNode(doc, { id: "I1", type: "concept", name: "New" }), /already exists/);
  });
});
...
describe("removeNode", () => {
  ...
  it("warns about operation references", () => {
    const doc: SysProMDocument = {
      nodes: [
        { id: "C1", type: "change", name: "Change", operations: [{ type: "update", target: "I1" }] },
        { id: "I1", type: "intent", name: "Intent" },
      ],
    };
    const result = removeNode(doc, "I1");
...
describe("updateNode", () => {
  it("updates specified fields", () => {
    ...
    const newDoc = updateNode(doc, "I1", { name: "New", description: "Updated" });
  ...
  it("rejects missing node", () => {
    const doc = makeDoc();
    assert.throws(() => updateNode(doc, "MISSING", { name: "New" }), /not found/);
  });
...
describe("addRelationship", () => {
  it("adds to relationships array", () => {
    const doc = makeDoc([{ id: "I1", type: "intent", name: "A" }, { id: "I2", type: "intent", name: "B" }]);
    const newDoc = addRelationship(doc, { from: "I1", to: "I2", type: "refines" });
...
describe("removeRelationship", () => {
  ...
  it("throws if relationship not found", () => {
    ...
    assert.throws(() => removeRelationship(doc, "I1", "refines", "I2"), /not found/);
  });
});
