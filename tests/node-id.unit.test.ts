...
describe("Node ID prefix validation", () => {
	it("accepts conforming numeric ID", () => {
		const result = Node.safeParse({ id: "DEC1", type: "decision", name: "Test" });
		assert.ok(result.success, `Expected valid: ${JSON.stringify(result.error?.issues)}`);
	});

	it("accepts conforming ID with descriptive suffix", () => {
		const result = Node.safeParse({ id: "INV28-CONF1", type: "invariant", name: "Test" });
		assert.ok(result.success);
	});

	it("accepts conforming ID with multi-segment suffix", () => {
		const result = Node.safeParse({ id: "STG1-DEC-PROPOSED", type: "stage", name: "Test" });
		assert.ok(result.success);
	});
