...
describe("relationships", () => {
  ...
  it("accepts additional properties on relationships (extensibility)", () => {
    valid({
      ...
      relationships: [
        { from: "A", to: "B", type: "depends_on", weight: 0.8 },
      ],
    });
  });
      ...
      nodes: [
        {
          ...
          subsystem: {
            ...
            relationships: [
              { from: "INV1", to: "I1", type: "constrained_by" },
            ],
          },
        },
      ],
...
describe("sysprom.spm.json", () => {
  it("validates against the schema", () => {
    ...
    assert.ok(result.success, "sysprom.spm.json should be a valid SysProM document");
  });
  ...
  it("relationships reference valid node IDs", () => {
    ...
    for (const rel of spmDocument.relationships) {
      assert.ok(ids.has(rel.from), `Relationship references unknown source: ${rel.from}`);
      assert.ok(ids.has(rel.to), `Relationship references unknown target: ${rel.to}`);
    }
  });
  ...
  it("every decision has options and a selected option", () => {
    ...
    for (const d of decisions) {
      assert.ok(d.options && d.options.length > 0, `Decision ${d.id} has no options`);
  ...
  it("every change references at least one decision via relationships", () => {
    ...
    for (const chId of changeIds) {
      ...
      assert.ok(linksToDecision, `Change ${chId} does not reference any decision`);
    }
  });

  it("decisions affecting domain nodes have must_preserve relationships", () => {
    const DOMAIN_TYPES = new Set(["intent", "concept", "capability", "element", "invariant"]);
  ...
  it("accepts a realistic SysProM document", () => {
    valid({
      ...
      nodes: [
        ...
        { id: "INV2", type: "invariant", name: "Placement-Agnostic Conversion" },
        { id: "EL1", type: "element", name: "Transformation Engine" },
        { id: "R1", type: "realisation", name: "Local Conversion", status: "active" },
        { id: "R2", type: "realisation", name: "Remote Conversion", status: "active" },
        {
          id: "D1",
          type: "decision",
        ...
        },
        {
          ...
          lifecycle: { defined: true, introduced: true, in_progress: false, complete: false },
        },
        {
          id: "V1",
          type: "view",
