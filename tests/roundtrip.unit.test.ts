...
import { markdownSingleToJson, markdownMultiDocToJson } from "../src/md-to-json.js";
import type { SysProMDocument } from "../src/schema.js";

function fixture(): SysProMDocument {
  return {
    metadata: { title: "Round Trip Test", doc_type: "sysprom", scope: "system", status: "active", version: 1 },
    nodes: [
      ...
      { id: "PR1", type: "principle", name: "Test Principle", description: "A value." },
      { id: "POL1", type: "policy", name: "Test Policy", description: "A rule." },
    ...
    external_references: [
      { role: "source", identifier: "https://example.com", node_id: "I1", description: "Source." },
    ],
  };
}

function assertNodesPresent(result: SysProMDocument, original: SysProMDocument) {
...
function assertRelationshipsPresent(result: SysProMDocument, original: SysProMDocument) {
  ...
  for (const orig of origRels) {
    ...
    assert.ok(found, `Missing relationship ${orig.from} -${orig.type}-> ${orig.to} after round trip`);
  }
}
...
describe("round trip: multi-doc", () => {
  ...
  it("JSON → MD → JSON preserves external references", () => {
    ...
    const ref = result.external_references.find((r) => r.identifier === "https://example.com");
        ...
        { id: "I1", type: "intent", name: "Parent Intent", description: "Parent." },
        {
          ...
          subsystem: {
            nodes: [
              ...
              { id: "INV1", type: "invariant", name: "Child Rule", description: "Must hold." },
            ],
          },
        },
      ],
