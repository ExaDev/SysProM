...
function check(doc: SysProMDocument): CheckResult {
  ...
  for (const node of doc.nodes) {
    ...
    if (node.type === "change" && (!node.scope || node.scope.length === 0)) {
      warnings.push(`${node.id}: change has no scope`);
    }

    // Changes without operations
    if (node.type === "change" && (!node.operations || node.operations.length === 0)) {
    ...
    if (node.scope) {
      for (const s of node.scope) {
        if (!ids.has(s)) {
          warnings.push(`${node.id}: scope references non-existent node ${s}`);
        }
      }
    }
  }
...
export function run(args: string[]): void {
  if (args.length < 1) {
    console.error("Usage: sysprom check <input> [--json]");
    process.exit(1);
  }
