...
export const json2mdCommand: CommandDef = {
  ...
  opts: z.object({
    singleFile: z.boolean().optional().describe("Force single-file output format"),
  }).strict(),
  action(args: unknown, opts: unknown) {
    ...
    const form =
      typedOpts.singleFile || outputPath.endsWith(".md") ? "single-file" : "multi-doc";
