...
function slugify(id: string, name: string): string {
  return `${id.toLowerCase()}--${name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/-$/, "")}`;
}
...
function generateReadme(doc: SysProMDocument, fromIdx: RelIndex): string {
  ...
  const fileDescriptions: Record<string, { label: string; role: string }> = {
    INTENT: { label: "Understand why this exists", role: "Enduring purpose, concepts, capabilities" },
    INVARIANTS: { label: "Understand what must always hold", role: "Rules that must hold across all valid states" },
    STATE: { label: "Understand what currently exists", role: "Current structure and active elements" },
    DECISIONS: { label: "Understand why things are the way they are", role: "Choices and rationale" },
    CHANGES: { label: "Understand how it has evolved", role: "Evolution over time" },
  };
  ...
  for (const [fileName, types] of Object.entries(NODE_FILE_MAP)) {
    if (doc.nodes.some((n) => types.includes(n.type))) {
      ...
      if (desc) presentFiles.push({ file: fileName, ...desc });
    }
  }
...
export function jsonToMarkdownMultiDoc(
  ...
  for (const n of subsystemNodes) {
    ...
    const slug = `${n.id}-${n.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/-$/, "")}`;
