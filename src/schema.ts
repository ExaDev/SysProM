...
function typedKeys<const T extends Record<string, string>>(obj: T): [keyof T & string, ...(keyof T & string)[]] {
  ...
  if (keys.length === 0) throw new Error("labelledEnum requires at least one entry");
  return [keys[0], ...keys.slice(1)];
}

function invertRecord<const T extends Record<string, string>>(record: T): Record<string, keyof T & string> {
